...
def find_E_F_angle(x,y):
    ...
    for i in range(0,2):
        ...
        res.append(np.arctan(np.divide(float(y_angle[i]),float(x_angle[i]))))

        res[i]=np.rad2deg(res[i])

        if x_angle[i] > 0 and y_angle[i] >0 :
            final_angle.append(270-res[i])
        if x_angle[i] < 0 and y_angle[i] >0 :
            final_angle.append(90-res[i])
        if x_angle[i] > 0 and y_angle[i] <0 :
            final_angle.append(90-res[i])
        if x_angle[i] < 0 and y_angle[i] <0 :
            final_angle.append(270-res[i])
...
def calibrate_gauge(gauge_number, file_type):
    ...
    imgout = cv2.imread('gauge-%s.%s' %(gauge_number, file_type))
    img = imgout.copy()
    img = 255 - img
